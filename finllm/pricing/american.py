...
class AmericanOption(Option):
    ...
    def binomial_tree(self, spot_price):
        ...
        p = (exp((self.rate - self.dividend_yield) * dt) - d) / (u - d) # Risk-neutral probability
        ...
        for step in range(steps -1, -1, -1):
            for i in range(step + 1):
                ST = spot_price * (u ** (step - i)) * (d ** i) # Stock price at node
                
                continuation_val = disc * (p * option_values[i] + (1 - p) * option_values[i + 1])
                if self.option_type == 'call':
                    exercise = max(0, ST - self.strike) # Exercise value
                else:
                    exercise = max(0, self.strike - ST) # Exercise value
